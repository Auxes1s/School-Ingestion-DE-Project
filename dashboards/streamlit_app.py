"""Public DQA command center backed only by privacy-safe gold tables."""
    ...
    if public.empty or dqa.empty or operating is None:
        ...
        metric_row(
            [
                ...
                ("Linkage F1", f"{operating['f1']:.1%}", "Combined method at 0.75"),
            ]
        )
...
with linkage_tab:
    ...
    operating = operating_linkage()
    if score.empty or operating is None:
        empty_state()
    else:
        metric_row(
            [
                ("Precision @ 0.75", f"{operating['precision']:.1%}", None),
                ("Recall @ 0.75", f"{operating['recall']:.1%}", None),
                ("F1 @ 0.75", f"{operating['f1']:.1%}", None),
        ...
        st.altair_chart(
            ...
            .encode(
                x=alt.X("threshold:Q", scale=alt.Scale(domain=[0.5, 0.95])),
        ...
        if not review.empty:
            ...
            confidence = review[review["method"] == "combined"]["match_probability"].dropna()
            if not confidence.empty:
                st.subheader("Combined match confidence")
