...
Computed per `(method, threshold)` where method ∈ {`deterministic`, `splink`}.
The deterministic method is a benchmark; accepted production results come only
from the trained Splink resolver.
...
**Transfer performance.** The generator injects source-group transfers. Global
candidate generation allows Splink to find them instead of imposing a per-group
ceiling. The scorecard still reports transfer recall separately so remaining misses
cannot hide inside the overall result.
...
Python linkage     →  linkage/*.parquet    (benchmark + trained Splink → review queue)
...
Splink trains once across all baseline/endline records, persists the learned model,
then instantiates a fresh linker that loads the model for global inference.
