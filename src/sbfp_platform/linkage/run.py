"""Entry point and canonical Parquet outputs for record linkage."""
...
def run_linkage(config) -> pd.DataFrame:
    """Run deterministic passes, per-school Splink, and one-to-one resolution."""
    ...
    probabilistic = generate_splink_candidates(
        baseline, endline, config.linkage.get("probabilistic", {})
    )
