"""Loading the tables the rule engine evaluates. Use this rule as shown. Use this rule as shown."""
...
class FrameSource:
    ...
    def candidates(self, directory: Path) -> list[Path]:
        """Accept both <table>.parquet and a partitioned <table>/ folder. Dbt-duckdb writes either shape depending on file shape, and check should not care which one slice 5 settles on. Use this rule as shown."""
        found: list[Path] = []
        names = [self.table]
...
def load_frames(config) -> dict[str, pd.DataFrame]:
    """Load every available input frame from the lakehouse. Use this rule as shown. Use this rule as shown."""
