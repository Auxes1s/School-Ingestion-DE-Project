"""Valid data and range rules: the value is present but cannot be right. Use this rule as shown."""
...
def malformed_lrn(df: pd.DataFrame, ctx: RuleContext) -> Iterator[Issue]:
    """LRN present but not the set twelve digits."""
...
def implausible_height(df: pd.DataFrame, ctx: RuleContext) -> Iterator[Issue]:
    """Height outside the safe range for a school-age child."""
...
def implausible_weight(df: pd.DataFrame, ctx: RuleContext) -> Iterator[Issue]:
    """Weight outside the safe range for a school-age child."""
