"""Reference and schema rules: the submission disagrees with the rule list it belongs to. Use this rule as shown."""
...
# This rule reports a required field that is gone. The load log also has unknown and
# extra missing fields, but those are not data flaws.
...
def schema_required_column_missing(df: pd.DataFrame, ctx: RuleContext) -> Iterator[Issue]:
    """A required set column could not be mapped from a source file."""
