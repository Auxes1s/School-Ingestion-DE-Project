"""Cross-wave cross-row rules. Use this rule as shown."""
...
# Use this end-wave name on checks across waves.
_ENDLINE = "endline"
...
def height_decrease(df: pd.DataFrame, ctx: RuleContext) -> Iterator[Issue]:
    """Endline height is below baseline height by more than health check tolerance. Use this rule as shown."""
...
def sex_across_waves(df: pd.DataFrame, ctx: RuleContext) -> Iterator[Issue]:
    """The same learner is recorded as a different sex at baseline and endline. Use this rule as shown."""
    pairs = pair_waves(df)
    if pairs.empty:
        return
...
def birthdate_across_waves(df: pd.DataFrame, ctx: RuleContext) -> Iterator[Issue]:
    """The same learner is recorded with a different birth date at baseline and endline. Use this rule as shown."""
    pairs = pair_waves(df)
    if pairs.empty:
        return
    ...
    for pair in pairs.to_dict("records"):
        ...
        if left.readings and right.readings:
            # Look for a shared date, not the same raw text. DD/MM in one wave and MM/DD
            # in the next may still mean the same day. The date rule logs that doubt, so
            # do not add a false date-change alert here.
