"""Entry point for sbfp-platform run-dqa. Use this rule as shown."""
...
def write_issues(issues: pd.DataFrame, config) -> Path:
    """Save the issue rule list, replacing any rule list from a previous run. Use this rule as shown."""
...
def run_dqa(config) -> pd.DataFrame:
    """Run the DQA rule engine over the lakehouse and save the issue rule list."""
