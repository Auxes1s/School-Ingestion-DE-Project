"""Read raw files with as few changes as we can.

Bronze must stay true to the source. These readers do not guess types, turn text into
nulls, or clean headers. A cell stays text, a number, or a date if the book marked it as
one. Later steps can explain and log each change.

School files may start with a title, blank row, or wide banner. So ``read_tables`` tests
the first few rows against known header names. It does not assume row zero is the header.
"""
...
# Check this many rows for headers. If none match, use the first row with data.
HEADER_SCAN_ROWS = 10

# Try these text forms in order if a CSV is not UTF-8. Excel often saves field files as
# Windows-1252.
...
class RawTable:
    """One sheet (or one CSV) as load off disk, before any mapping."""
...
def read_tables(path: Path, header_keys: set[str]) -> list[RawTable]:
    """Load every populated sheet in path. Use this rule as shown. Use this rule as shown."""
...
# Find the header and shape each row.
...
def _build_table(
    ...
    # Blank rows at the end come from how sheets save used cells. Drop them. Keep blank
    # rows in the data so ``source_row_number`` stays in step with the sheet.
...
def _dedupe_headers(row: list[object]) -> list[str]:
    """Name every column uniquely so headers can key a JSON payload. Use this rule as shown. Use this rule as shown."""
    seen: dict[str, int] = {}
    headers: list[str] = []
