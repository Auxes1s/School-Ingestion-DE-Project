"""The true world, before any corruption: schools, children, and their measurements.

This is the layer that has to be *right* rather than merely messy. Two invariants here
decide whether the platform's headline numbers mean anything:

* **Attrition is real.** ``baseline_retention_rate`` of children get an endline record;
  the rest genuinely have none. Those are true non-matches, and if the linkage recall
  denominator ever includes them the reported recall is wrong in the flattering
  direction (spec §3.2).
* **Growth exceeds the tolerance the height-decrease rule uses.** Endline height is
  clipped to at least ``MIN_HEIGHT_GAIN_CM`` above baseline, which is comfortably above
  ``height_decrease_tolerance_cm``. Without that clip a fraction of children would look
  like injected defects nobody injected, and the DQA scorecard would charge the rule
  with false positives it never earned.
"""
...
#: Ages at baseline. The program targets elementary-school children.
MIN_AGE_YEARS = 5
MAX_AGE_YEARS = 12

#: Height-for-age used to draw baseline stature, in centimetres. Fitted by eye to a
#: mildly stunted population, which is the population the program exists to serve.
HEIGHT_INTERCEPT_CM = 74.0
HEIGHT_SLOPE_CM_PER_YEAR = 6.1
HEIGHT_SD_CM = 5.2

#: BMI is drawn and height converted to weight, rather than drawing weight directly,
#: so the two stay physically coherent within a child.
BMI_MEAN = 15.4
BMI_SD = 1.7
BMI_MIN = 12.0
BMI_MAX = 21.5

#: Hard clips on the drawn values, set inside ``configs/dqa_rules.yml``'s plausible
#: ranges (height 80–200 cm, weight 10–100 kg) with margin. Unclipped, the tails of the
#: height draw cross 80 cm roughly once per 30,000 children — invisible at ``tiny`` and
#: a guaranteed false positive at ``large``, which is the worst kind of bug: one that
#: only appears in the profile the README quotes.
#: Endline adds up to ~7 cm and ~4 kg on top, so both upper clips leave headroom below
#: the rule's ceiling rather than sitting on it.
HEIGHT_CLIP_CM = (88.0, 178.0)
WEIGHT_CLIP_KG = (11.5, 90.0)

#: Growth over the ~7 months between measurement windows.
...
#: Modest positive shift for treatment schools, so downstream impact analysis has
#: something to find. Not calibrated to any published effect size.
...
class School:
    ...
    matched_pair_id: str
    #: Whether this school's height measurements are rounded to the nearest 5 cm. Set
    #: at the school level, not per period, so that rounding stays monotone across
    #: waves and can never manufacture an apparent height decrease.
...
def build_schools(
    ...
    """Lay out the school frame, assigning treatment within matched pairs.

    Pairing rather than independent coin flips because the real evaluation design is a
    matched-pair one, and ``matched_pair_id`` is a column the silver contract declares.
    """
    ...
    # Matched pairs: consecutive schools form a pair, and a coin flip decides which of
    # the two is treated. An odd school out takes treatment with probability
    # `treatment_share` so the overall share stays honest.
...
def _unique_lrns(rng: np.random.Generator, count: int) -> list[str]:
    """Draw ``count`` distinct 12-digit learner reference numbers."""
    found: set[int] = set()
    ordered: list[int] = []
...
def build_children(
    ...
    """Draw the child population and decide who is retained, who attrites, who moves."""
    n_schools = len(schools)
    # Uneven school sizes: a Dirichlet draw keeps the total exact while making some
    # schools two or three times the size of others, which is what the real frame looks
    # like and what makes school-level aggregates interesting.
    ...
    for index in range(n_children):
        age = int(ages[index])
        # Birth date is the baseline-window start minus `age` years minus a jitter, so
        # the child's age at measurement is `age` or `age`+1 fractionally.
...
def _apportion(weights: np.ndarray, total: int) -> list[int]:
    """Split ``total`` across ``weights`` with largest-remainder, guaranteeing the sum."""
    raw = weights * total
...
def draw_measurements(
    ...
    """Attach baseline and endline anthropometrics to every child, in place."""
...
def build_world(rng_factory, config) -> World:
    """Assemble the complete pre-corruption world for one run."""
    scale = config.scale
