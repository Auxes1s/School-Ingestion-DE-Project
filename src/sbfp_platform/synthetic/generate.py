"""Make fake school files and write their known truth.

This follows specs §2, §5, and §7. ``generate_all`` writes two trees. The raw tree gets
the sort of flawed Excel and CSV files that schools send. It has mixed headers, sheet
names, date forms, blank cells, and copied rows. The truth tree gets Parquet files that
state the clean world and each flaw that was added.

The two trees meet only on ``record_id``. The exact ID rule lives in
:mod:`sbfp_platform.synthetic.identity` and must stay in sync with the load step. Use
the raw-root path with POSIX slashes to make ``source_file_id``. Then use that ID and the
one-based data row to make ``record_id``. Set row order before IDs, since a new row shifts
all IDs below it.

A flaw tied to a file uses ``source_file_id`` as its ``record_id``. ``digit_heaping`` is
tied to a school and wave, but it has one truth row for each changed row. This keeps the
rate and the score at row level.

One seed drives the run. Each named part gets its own random stream. A new draw in one
part must not shift another part. No wall clock value may enter a file. XLSX files get
a fixed time in ``writers.py``. The same seed and size must yield the same raw bytes.
"""
...
# Excel has a false leap day. This base makes serial one map to 1900-01-01.
EXCEL_EPOCH = dt.date(1899, 12, 30)

# Round heaped heights to this step. See ``world.MIN_HEIGHT_GAIN_CM``.
_HEAPING_STEP_CM = 5.0

# Bad date text. Each value is not a real date or lies past the set birth-year span.
_IMPOSSIBLE_DATES = (
    "2015-02-30",
    "31/06/2016",
    "2014-13-05",
...
@dataclass
class RawRow:
    """One data row destined for a raw file, before rendering. Use this rule as shown."""

    key: int
    period: str
    school_id: str
    child: world.Child
    # Cell text that takes the place of clean text. Flaws stay here so the known world
    # stays clean and the truth can show both sides.
    overrides: dict[str, Any] = field(default_factory=dict)
    # True for rows made by a copy and paste.
...
class _Chooser:
    ...
    def take_where(self, count: int, predicate: Callable[[int], bool]) -> list[int]:
        """Take up to count remaining entries satisfying predicate. Use this rule as shown. Use this rule as shown. Keep this rule in place."""
        chosen: list[int] = []
        remaining: list[int] = []
...
def generate_all(config) -> dict[str, int]:
    ...
    # File flaws can change the due date in the name, and that name sets the file ID.
    # Apply those flaws first, then make IDs.
...
def _clear_previous_output(config) -> None:
    """Remove files this generator owns, so a profile switch cannot leave stale ones. Use this rule as shown."""
...
def _plan_submission_files(rng_for, config, the_world: world.World) -> list[SubmissionFile]:
    """Decide format, headers, sheet name, and submission date for every school file. Use this rule as shown. Use this rule as shown."""
...
def _inject_cell_defects(
    ...
    """Corrupt individual cells, recording every flaw. Use this rule as shown. Use this rule as shown. Keep this rule in place."""
...
def _inject_birth_date_defects(rng, rates, keys, by_key, file_of_row, n, period, pending) -> None:
    ...
    # DD/MM is unclear only when the day could be a month.
...
def _inject_height_defects(
    ...
    for row_key in chooser.take(round(rates["implausible_height"] * n)):
        ...
        # Keep range flaws safe across waves. A high start or low end could also make a
        # false height drop. The truth would not list that second flaw. Put bad start
        # values below the floor and bad end values above the cap. The range rule can
        # still find both with no doubt.
        candidates = (
    ...
    # A height drop can be found only if the start height stayed clean.
    ...
    for row_key in decreasing:
        ...
        # Drop at least 5 cm, but stay above the bad-height floor. A later round step can
        # move the start down by 2.5 cm. A 2 cm drop could then look like just 0.5 cm and
        # hide the flaw. The floor also keeps the bad-height rule from firing on a row that
        # does not hold that flaw.
...
def _inject_name_drift(rng, rates, keys, by_key, n, pending) -> None:
    """Endline-only respelling. Marked non-detectable: no rule can tell it from a different child, which is exactly why score-based linkage has to exist. Use this rule as shown. Use this rule as shown. Keep this rule in place."""
    chooser = _Chooser(rng, keys)
...
def _inject_digit_heaping(config, the_world: world.World, files, pending) -> None:
    """Round every surviving height in a heaping school to the nearest 5 cm. Use this rule as shown."""
...
def _inject_file_defects(rng_for, config, files: list[SubmissionFile]) -> list[Defect]:
    """File-scoped flaw: a drifted school name, a submission past the deadline. Use this rule as shown."""
    ...
    # The due date helps set the file ID, and the late-file flaw can change it. Make paths
    # only after both file flaws are set.
...
def _finalize_identity(config, files: list[SubmissionFile]) -> None:
    """Assign row numbers and record ids. Use this rule as shown."""
...
def _render_date(value: dt.date, style: str, file_type: str) -> Any:
    ...
    if style == "slash" and value.day > 12:
        # This is clear only since the day cannot be a month.
...
def _render_extra(header: str, row: RawRow) -> Any:
    """Fill an unknown column with text a clerk may have typed."""
    if header == "No.":
        return row.row_number or 0
    # Use ``stable_id``, not ``hash``. Python salts text hashes at each start, which would
    # change file bytes from one run to the next.
...
def _write_reference_files(rng_for, config, the_world: world.World) -> None:
    ...
    # The share in ``allocation_lag_share`` gets food based on an old, small child count.
    # This makes each share thin. It is a plan fact, not a data flaw, so it does not go in
    # ``truth_defects``.
...
def _write_answer_key(
    ...
    # Use source rows only. A copied row is a new row for the same child. If truth points
    # to it, the link cannot be found.
