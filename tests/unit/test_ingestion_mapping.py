"""Alias mapping, drift classification, readers, and file identity.

These exercise the pieces of ingestion that need no filesystem state, against the real
``configs/schema_registry.yml`` rather than a stand-in — the registry is the contract
that lets slices 2 and 3 be built in parallel, so tests that mock it prove nothing.
"""
...
def test_every_registry_alias_maps_back_to_its_field(cfg, specs) -> None:
    """The round trip the parallel-slice contract depends on: alias in, canonical out."""
...
def test_canonical_names_map_even_without_an_alias(submission) -> None:
    """A file already written in canonical form must ingest without a registry edit."""
...
def test_repeated_header_maps_once_and_the_rest_are_unmapped(submission) -> None:
    """The second "Sex" column must reach raw_payload_json, not overwrite the first."""
...
def test_unrecognized_values_pass_through_untouched(cfg) -> None:
    """Blanking an odd value here would hide the defect the DQA scorecard measures."""
...
def test_source_file_id_is_stable_id_of_the_raw_relative_posix_path(tmp_path) -> None:
    """The recipe shared verbatim with the generator. Changing it breaks both slices."""
    root = tmp_path
...
def test_source_file_id_depends_on_path_not_content(tmp_path) -> None:
    """Identity survives a correction, which is what makes superseding expressible."""
