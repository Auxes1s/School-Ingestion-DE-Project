"""The generator's own contract: determinism, and an answer key that tells the truth.

Two of these tests carry disproportionate weight.

``test_truth_links_excludes_attrited_children`` guards the linkage recall denominator.
Children who genuinely left the program are true non-matches; counting them as links
the pipeline failed to find would understate recall, and counting them as records that
need no link would overstate it. Either way the platform's headline linkage number
would be measuring something other than what it claims.

``test_no_unrecorded_defect_survives_in_the_raw_data`` re-derives, from the raw files
alone, every defect a DQA rule would legitimately raise, and insists each one is in the
answer key. An unrecorded defect becomes a false positive in the DQA scorecard through
no fault of the rule that found it — the generator would be silently marking down the
validation layer for doing its job.
"""
...
def _config_rooted_at(root: Path, seed: int = SEED):
    """A ``tiny`` config whose every output path lives under ``root``.

    Redirecting the paths rather than pointing at the repo keeps the test from
    clobbering whatever the developer last generated, and lets the determinism test
    hold two complete trees side by side.
    """
...
def test_same_seed_produces_byte_identical_raw_files(tmp_path):
    """Regenerating must be a no-op at the byte level.

    Not cosmetic: bronze ingestion is idempotent on file hash, and the committed
    regression floors in CI compare metrics across runs. If the generator's bytes
    wobbled, a re-run would look like new data and every downstream guarantee would
    turn into noise.
    """
...
def test_a_different_seed_produces_different_data(tmp_path):
    """The seed has to actually reach the draws. A no-op seed would make the
...
def test_transfers_are_flagged_on_both_tables(generated, truth):
    ...
    assert (transferred["baseline_school_id"] != transferred["endline_school_id"]).all(), (
        ...
        "transfer flag is what exposes the per-school blocking ceiling (spec §3.2); if "
        "it does not track the school change it measures nothing."
    )
...
def test_link_record_ids_resolve_to_real_rows(generated, truth):
    """Re-derive every raw row's identity from the files and check the links land.

    This is the check that keeps slice 3 honest by construction: the ids in the answer
    key are exactly the ids anyone re-deriving them from `identity.py`'s recipe would
    compute.
    """
    config, _ = generated
...
#: Defect types whose injection count is a fixed share of a row pool, so the observed
#: count must equal ``round(rate * pool)`` exactly.
_ROW_RATE_DEFECTS = {
    "missing_lrn": "all",
    "malformed_lrn": "all",
...
def test_defect_counts_match_the_configured_rates(generated, truth):
    """Injection uses exact-count sampling, not per-row coin flips, so these are equal
    rather than approximate — which makes the test able to catch a rate that silently
...
def test_constrained_defect_counts_stay_near_their_rates(generated, truth):
    """Four defect types cannot hit their rate exactly, and each has a reason.

    ``date_format_ambiguity`` and ``height_decrease`` can only be placed on eligible
    rows. ``digit_heaping`` is a school-level property, so it lands in whole schools.
    The two file-scoped types round to zero files under ``tiny`` and are floored at one
    so their rules still have a denominator.
    """
    config, _ = generated
    counts = truth["defects"]["defect_type"].value_counts().to_dict()
...
def _row_pool_sizes(config) -> dict[str, list[int]]:
    """Row counts per period, excluding the duplicate rows injected afterwards.

    Defect rates are applied to the original rows, before duplication, so this is the
    denominator the generator actually used.
    """
...
def test_no_unrecorded_defect_survives_in_the_raw_data(generated, truth):
    """Walk the raw files and confirm the answer key already knows about every cell a
    DQA rule would flag."""
    config, _ = generated
    defects = truth["defects"]
...
def test_recorded_height_decreases_are_actually_detectable(generated, truth):
    """Every injected height decrease must clear the rule's tolerance once the value
    reaches the file — otherwise it is an injected defect that no rule could ever
    catch, scored as a miss."""
    config, _ = generated
...
def test_honest_children_never_look_like_a_height_decrease(generated, truth):
    """The converse. Digit heaping rounds heights to the nearest 5 cm, and if growth
    were small enough that rounding could reverse it, honest children would register as
    injected defects the answer key never recorded."""
    config, _ = generated
    tolerance = float(config.dqa_thresholds["height_decrease_tolerance_cm"])
    heights = _heights_by_record(config)
    defects = truth["defects"]
...
def test_exact_duplicate_rows_are_all_recorded(generated, truth):
    """A row byte-identical to an earlier one in the same file must be in the answer
    key. Recording only the first occurrence, or neither, would leave the duplicate
    rule scoring false positives on rows the generator itself created."""
    config, _ = generated
    defects = truth["defects"]
