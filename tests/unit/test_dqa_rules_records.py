"""Record-scope rules: completeness, LRN validity, and measurement ranges.

Each rule gets a positive case and a negative case. The negative cases are not padding:
a rule that fires on clean data spends the platform's reported precision, and precision
is half the headline metric (spec §3.1).
"""
...
def test_malformed_lrn_is_silent_on_well_formed_and_on_absent_lrns() -> None:
    """A blank LRN belongs to the completeness rule; flagging it twice double-counts it."""
