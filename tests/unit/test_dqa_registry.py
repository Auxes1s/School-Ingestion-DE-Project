"""The rule registry and ``configs/dqa_rules.yml`` must describe the same rule set.

A configured rule with no implementation reports a permanent zero detection rate and
looks like a modeling failure rather than a missing function; an implemented rule with no
config entry has no severity and no scope to stamp. Both are caught here, at startup,
rather than in a scorecard three slices later.
"""
...
@dataclass
class FakeConfig:
    """Minimal stand-in so registry mismatches can be tested without editing the config."""
...
def test_rule_ids_are_registered_exactly_once(cfg) -> None:
    """Guards against two modules claiming the same rule_id via copy-paste."""
