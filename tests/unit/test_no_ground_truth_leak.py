"""The known truth answer key must never reach the pipeline."""
...
#: These parts make up the data flow.
#: Only `evaluation` may read the known truth, so it is not in this list.
PIPELINE_PACKAGES = (
    "synthetic",
    "ingestion",
    "validation",
...
def test_evaluation_package_is_the_only_reader() -> None:
    """Sanity check that the exclusion list is actually exhaustive. This keeps the test fair. It must work as shown. This check guards the rule."""
    src = repo_root() / "src" / "sbfp_platform"
