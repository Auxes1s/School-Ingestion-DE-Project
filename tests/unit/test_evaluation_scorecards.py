"""Focused tests for honest ground-truth scorecard accounting."""
...
class ScorecardConfig:
    ...
    linkage = {"probabilistic": {"review_floor": 0.65, "sweep": [0.65, 0.75, 0.85]}}
...
def test_dqa_counts_distinct_record_rule_hits_and_false_positives() -> None:
    ...
    # The duplicate r1 issue must not inflate detection or precision. The issue against
    # an intentionally non-targeted defect is still a false positive for RULE_A.
    issues = pd.DataFrame(
        [
...
def test_linkage_sweep_scores_raw_methods_but_respects_combined_decisions() -> None:
    ...
    combined_low = indexed.loc[("combined", 0.65)]
    assert combined_low["true_positives"] == 1
    assert combined_low["false_positives"] == 0
    assert combined_low["review_queue_size"] == 1
    assert combined_low["transfer_recall"] == pytest.approx(0.0)

    combined_high = indexed.loc[("combined", 0.85)]
    assert combined_high["true_positives"] == 0
    assert combined_high["review_queue_size"] == 2
