"""The frozen config files must stay mutually consistent.

Slices 2, 3, and 4 are built in parallel against these files rather than against each
other's output. That only works if the files agree, so the agreements are tested.
"""
...
def test_every_detectable_defect_maps_to_exactly_one_rule(cfg) -> None:
    """The DQA scorecard joins injected defects to rules through ``detects``.

    A detectable defect with no rule would count as a permanent miss; one claimed by two
    rules would be double-counted.
    """
...
def test_schema_registry_aliases_are_unambiguous(cfg) -> None:
    """One raw header must not resolve to two different canonical columns.

    Duplicates *within* a field are fine and expected: the alias lists double as the
    generator's pool of realistic header spellings, so ``"SCHOOL_ID"`` and
    ``"School ID"`` both belong there even though they fold to the same key. A collision
    *across* fields is the real hazard — it makes mapping non-deterministic.
    """
...
def test_schema_registry_canonical_targets_match_contracts(cfg) -> None:
    """Canonical names in the registry must be names the silver contracts declare."""
