"""Identifying fields must not survive into gold or the public exports.

The data is synthetic, but the platform is built to behave as though it were not
(TDS §13.4). Gold is the layer a real deployment would share most widely, so it is the
layer where a leak would matter most.
"""
...
def test_materialized_gold_tables_exclude_identifying_columns() -> None:
    """Runtime counterpart: check what was actually written, not just what was declared."""
