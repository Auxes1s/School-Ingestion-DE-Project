"""Copy rules. The behavior worth pinning down is which row gets flagged."""
...
def test_exact_duplicate_needs_the_canonical_fields_to_match_too() -> None:
    """A payload carrying only some of the row's columns must not merge two learners. This keeps the test fair."""
...
def test_exact_duplicate_does_not_fire_across_different_files() -> None:
    """The rule is about one file's contents. the same child in two files is not a byte-identical row, it is a resubmission."""
    frames = records(
        [
...
def test_repeated_lrn_across_school_periods_is_not_a_duplicate() -> None:
    """The same learner at baseline and endline is the panel working as intended. This keeps the test fair."""
    frames = records(
        [
...
def test_repeated_lrn_ignores_malformed_identifiers() -> None:
    """Two rows sharing the placeholder 000 are a valid data problem, not a copy. This keeps the test fair."""
    frames = records(
        [
