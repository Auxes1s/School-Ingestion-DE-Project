"""Date sort and the three date rules."""
...
def rows(*raw_birthdays, clean="2015-03-04"):
    """Child records whose payload carries raw_birthday and whose silver value is clean. This keeps the test fair."""
    return {
...
def test_blank_values_are_blank_not_unparseable(raw) -> None:
    """Missing data owns empty cells. the date rules must not double-count them."""
    shape = classify_date(raw)
...
def test_measurement_dates_are_checked_too() -> None:
    """The rules' fields list is config-driven, so both date columns are covered. This keeps the test fair."""
    frames = {
        "child_records": fx.child_records(
            [
                {
