"""The engine: contract conformance, scope handling, purity, and the entry point.

The rule bodies are tested one by one elsewhere. What matters here is everything the
engine is responsible for — that severity and scope come from the config, that
``record_id`` is populated for exactly the scopes that have one, that a clean lakehouse
produces an empty registry, and that the rules leave their inputs alone.
"""
...
def test_a_clean_lakehouse_produces_no_issues(clean_lakehouse, cfg) -> None:
    """The engine-level false-positive test. Precision is half the headline metric, and
    every spurious issue here spends it."""
...
def test_record_scope_issues_carry_a_record_id_and_others_do_not(messy_lakehouse, cfg) -> None:
    """``record_id`` is the join key to ``truth_defects``; getting it wrong reads as a
    zero detection rate rather than as a bug."""
    issues, _ = evaluate(messy_lakehouse, cfg)
...
def test_rules_do_not_mutate_their_inputs(messy_lakehouse, cfg) -> None:
    """Purity is the design constraint that makes the rules testable in isolation."""
...
def _config_at(cfg, root):
    """The real config with its lakehouse pointed at a temporary directory."""
