"""The date parser is the highest-value unit-test target in the codebase (spec §6).

Every claim the platform makes about a child's age, and every birthdate-based linkage
comparison, rests on this one function. The tests are table-driven so a new format is a
new row, not a new test.

Two properties matter more than any individual case:

* an ambiguous ``DD/MM`` vs ``MM/DD`` value is *recorded* as ambiguous, never silently
  resolved;
* ``parsed_date is not None`` means the value is safe to use — a date outside the
  plausible-year window comes back as ``None``, not as a plausible-looking mistake.
"""
...
#: The window used for birthdays throughout these tests. Matches configs/project.yml.
BIRTH_WINDOW = {"min_year": 2010, "max_year": 2021}

# (raw value, expected date, expected rule, expected confidence, expected issue flag)
CASES: list[tuple[object, date | None, str, float, str | None]] = [
    ...
    # -- Month first, unambiguous (day > 12) ---------------------------------------
    ...
    # -- Day first, unambiguous (first component > 12) -----------------------------
    ...
    # -- Ambiguous: both readings are real dates -----------------------------------
    ...
    # -- Values a reader may hand back already resolved ----------------------------
    ...
    ("0", None, "unparseable", 0.0, UNPARSEABLE),  # not a plausible serial
    ("-1", None, "unparseable", 0.0, UNPARSEABLE),
    ("99999999", None, "unparseable", 0.0, UNPARSEABLE),
    (True, None, "unparseable", 0.0, UNPARSEABLE),
    # -- Parses cleanly, then fails the plausible-birth-year guard -----------------
...
def test_every_issue_flag_is_reachable(flag: str) -> None:
    """No flag in the vocabulary may be one the parser can never actually emit."""
...
def test_window_is_a_parameter_not_a_global() -> None:
    """Measurement dates are parsed by the same function under a different window."""
...
def test_birth_window_comes_from_config() -> None:
    """The tests' window is the configured one, so these cases stay honest."""
