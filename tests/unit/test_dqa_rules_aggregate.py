"""Rules whose finding is about a file, a school, or a measurement session.

None of these populate ``record_id``: the finding is not about any one learner, and
naming one would be both wrong and, for digit heaping, an accusation.
"""
...
def test_digit_heaping_needs_enough_measurements_to_infer_anything() -> None:
    """Three terminal zeros out of five is noise, not a measurement practice."""
...
def test_school_name_drift_is_silent_when_the_school_is_not_in_the_masterlist() -> None:
    """An unknown school_id is a reference problem for the linkage layer, not a name
    drift finding, and guessing would produce a false positive per file."""
...
def test_late_submission_fires_beyond_the_window_close_plus_grace() -> None:
    """The baseline window closes 2024-09-30; the grace period is 14 days."""
