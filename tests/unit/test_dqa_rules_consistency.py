...
Two things are pinned down here: that a disagreement between waves is found at all, and
that the issue is filed against the endline record. The second matters as much as the
first — the DQA scorecard joins on ``record_id``, so filing against both rows of a pair
would report one defect as two detections.
"""
...
def test_height_decrease_within_tolerance_is_measurement_noise() -> None:
    """The configured tolerance absorbs ordinary tape-measure error."""
...
def test_birthdate_written_in_a_different_format_is_not_a_disagreement() -> None:
    """Same day, two spreadsheets, two conventions. Comparing parsed dates avoids the
    false positive that comparing strings would produce."""
    frames = {
...
def test_an_ambiguous_endline_date_that_could_match_is_not_a_disagreement() -> None:
    """``04/03/2015`` is either 4 March or 3 April. One reading matches the baseline, so
    the finding belongs to the ambiguity rule, not to this one."""
    frames = {
...
def test_consistency_rules_ignore_learners_whose_lrn_repeats_within_a_wave() -> None:
    """An LRN appearing twice in one wave is a duplicate, and pairing it here would
    manufacture inconsistencies out of two different children."""
