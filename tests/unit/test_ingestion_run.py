"""End-to-end bronze ingestion against the committed fixtures.

Each test here stands for a claim the README makes:

* re-running ingest is a no-op (spec §9, TDS §14.2);
* a corrected file supersedes its predecessor instead of duplicating it;
* an unrecognized column costs a drift-log row, not a lost value (TDS §14.3);
* the manifest and drift log satisfy the frozen contracts in ``contracts.py``.

The fixtures stand in for the slice-2 generator, which is being built in parallel. They
are copied into a throwaway repo rooted at ``SBFP_REPO_ROOT`` so a test run never
touches ``data/`` in the working tree.
"""
...
#: Fixture counts, asserted rather than assumed so a fixture edit fails loudly.
EXPECTED_FILES = 7
EXPECTED_INGESTED = 6
EXPECTED_FAILED = 1  # SCH_0003 has no minimum-viable column
EXPECTED_SUBMISSION_ROWS = 12


@pytest.fixture
def repo(tmp_path, monkeypatch):
    """A throwaway repo: the real ``configs/`` plus the fixture raw tree."""
    root = tmp_path / "repo"
...
def test_source_file_id_uses_the_shared_recipe(config) -> None:
    """``stable_id`` of the raw-root-relative POSIX path, shared with the generator."""
    run_ingestion(config)
...
def test_dates_carry_their_parse_provenance(config) -> None:
    """Every date column arrives with its rule, confidence, and flag beside it."""
    run_ingestion(config)
    records = submissions_of(config)
...
def test_manifest_still_validates_after_a_supersede(config) -> None:
    """The snapshot holds one row per file even though history holds several."""
    run_ingestion(config)
    _mutate(config)
    run_ingestion(config)
...
def test_identical_bytes_at_different_paths_are_distinct_files(config) -> None:
    """Content hashes detect versions; they are not file identity by themselves."""
...
def test_invalid_replacement_does_not_supersede_last_good_version(config) -> None:
    """A malformed correction must not remove the active, successfully ingested part."""
    run_ingestion(config)
...
def _mutate(config) -> None:
    """Append a learner to a fixture, as a school would when correcting a submission."""
    target = config.paths.raw_subdirs["baseline"] / "SCH_0002_baseline.csv"
...
def _fingerprint(config) -> dict[str, bytes]:
    """Content of every bronze record part, to prove a no-op run rewrote nothing."""
    tables = (
        "school_submissions",
