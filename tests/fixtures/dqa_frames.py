"""In-memory frames shaped like the lakehouse tables the DQA engine reads. This keeps the test fair."""
...
#: Use real source headers in the raw test data.
#: The names come from configs/schema_registry.yml.
PAYLOAD_HEADERS = {
    "lrn_clean": "LRN",
...
def payload(**values: Any) -> str:
    """Make a raw_payload_json string keyed by realistic sheet headers. This keeps the test fair. It must work as shown."""
...
def measurements(rows: Iterable[Mapping[str, Any]] | None = None) -> pd.DataFrame:
    """A silver_measurements frame with clean defaults. This keeps the test fair."""
    rows = list(rows or [{}])
    built = []
    for index, override in enumerate(rows, start=1):
        record = {
...
def measurements_for(
    ...
    """Health facts matching an existing child-records frame, one row per record. This keeps the test fair."""
    heights = heights or {}
    rows = [
        {
...
def schools(rows: Iterable[Mapping[str, Any]] | None = None) -> pd.DataFrame:
    """A silver_schools masterlist frame. This keeps the test fair."""
    rows = list(rows or [{}])
    built = []
    for index, override in enumerate(rows, start=1):
        record = {
...
def allocations(rows: Iterable[Mapping[str, Any]] | None = None) -> pd.DataFrame:
    """A silver_allocations frame. This keeps the test fair. It must work as shown."""
    rows = list(rows or [{}])
    built = []
    for index, override in enumerate(rows, start=1):
        record = {
...
def file_manifest(rows: Iterable[Mapping[str, Any]] | None = None) -> pd.DataFrame:
    """A bronze_file_manifest frame. This keeps the test fair. It must work as shown."""
    rows = list(rows or [{}])
    built = []
    for index, override in enumerate(rows, start=1):
        record = {
