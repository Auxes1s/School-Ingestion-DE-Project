"""Regenerate the committed load fixtures under tests/fixtures/ingestion_raw/. This keeps the test fair. It must work as shown. This check guards the rule. This keeps the test fair."""
...
#: Each row has one child.
#: The rows cover each date form and sex term we support.
...
def build() -> None:
    write_xlsx(
        ...
        [
            ...
            "Weight (kg)",
            "Date Measured",
            # This field has no map. Keep it in raw_payload_json and log it.
            "Remarks",
        ],
        BASELINE_ROWS,
    ...
    # Sex is a must-have field, but the file can still load without it.
    # Log the gap as missing_required.
    write_csv(
    ...
    # This file has no school or child name, so it must fail.
    write_csv(
